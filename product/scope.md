...
> ([§8 Recommended Product Scope](#8-recommended-product-scope)) as the working
> roadmap; individual CF/SF items are re-validated when a session picks them up.
...
> recommendations, not commitments, and the
> [§8 phase plan](#8-recommended-product-scope) is the research-era sketch,
> annotated in place below. This supersedes the provenance note above where
> it calls that phase plan "the working roadmap".
>
> **C12 addition (owner decision, 2026-08-18):** the phase groupings in this
> document — [§8](#8-recommended-product-scope) and the "research phase n"
> language that used to appear throughout — were **arbitrary reading-order
> clustering from the research phase**; they never carried planning
> meaning. Exactly one plan-phase ever existed: the 7-day prototype
> ([roadmap.md](roadmap.md)). The **targeted
> version** — what Marginalia v1 is, item by item — is defined in
> [target-scope.md](target-scope.md). This file keeps its path and its CF/SF
> ids so recorded citations (handovers, PR bodies, the security register's
> scope-doc anchors) continue to resolve.
>
> **C13 addition (owner decision, 2026-08-19):** this document no longer
> tracks implementation status at all. The per-item status annotations and
> the summary status table that sessions C7/C10/C12 maintained here have
> been **removed**: what Marginalia v1 delivers, how far, and with which
> shipping PRs is recorded in exactly one place,
> [target-scope.md](target-scope.md),
> measured against the v1 target rather than against this catalog. What
> stands below is the description of Gemini Notebook's functionality — the
> research result, and nothing about our implementation. Everything the
> prototype deliberately does not do stays recorded on the
> [roadmap's cut list](roadmap.md).
...
> **Where the non-functional record lives (2026-08-18, restated 2026-08-19):**
> the NF sections below are research requirements, not commitments. What the
> deployed system actually does about them is recorded in the security
> register (`product/security.md`, repo-only), the
> [feasibility register](feasibility.md) (decisions D-1…D-10 and risk rows),
> and the [architecture views](architecture/index.md), which describe the
> system as deployed — plus [target-scope.md](target-scope.md) for the
> quotas and guards that Marginalia v1 commits to.
...
>
> The CF/SF ids behind each phase row, made explicit here:
> **Phase 1** — CF-01, CF-02 (PDF/doc/text/web
...
> clustering from the research phase**, not stages of any plan. The only
...
> [target-scope.md](target-scope.md). This blockquote is the one place the
> clustering is explained; nothing else in the document depends on it.
