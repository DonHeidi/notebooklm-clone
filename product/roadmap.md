...
| C13 | `docs/target-scope-completion` | Target-scope rework (owner review 2026-08-19): statuses re-baselined **against the target, not the research catalog** (a known limitation flags 🔶 only if it sits inside the target definition — e.g. email+password auth IS the target → ✅ with limitation noted); each row states what the target actually is; the "badge" shorthand replaced with stated limitations or real links; row set completed from what the roadmap sessions delivered (static sites etc. are missing today). | C12 |
