...
| C8 | `docs/history-latest` | Bring `product/history/*` current through the fully-merged board — per-file gaps: webapp (A6, A7, D2), infrastructure (B4, B5), supabase (B5), docs (C5–C7), process (A4–A6, B3–B5, C5, C7, D2) — update the architecture views (physical, development) for B5's Supabase-under-Terraform, and add a **generative view** to the architecture docs (owner-requested 2026-08-18): model inventory (which model, where, when, what for — chat, embeddings, TTS), the RAG pipeline end-to-end, derived from code not memory. Sourced from handovers/PR record, same method as C4/C5. Added 2026-08-18. | C4–C6; all sessions merged (they are) |
