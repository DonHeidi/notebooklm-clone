...
| C13 | `docs/target-scope-rebaseline` | Target-scope rework (owner review 2026-08-19): the C12 table misrepresents a delivered product as half-implemented — statuses were mirrored from the research catalog. Re-baseline **against the target** (a limitation flags 🔶 only if it sits inside the target definition — e.g. email+password auth IS the target → ✅ with limitation noted); each row states what the target actually is (renamed where the name was the catalog's title, not the target's); the "badge" shorthand replaced with stated limitations or real links. **Owner decision 2026-08-19 (supersedes the glyph fix):** the research catalog drops implementation-status tracking entirely — Status/Evidence columns and per-item status badges removed from `scope.md`; status and evidence live only in `target-scope.md`. *(Row text corrected 2026-08-19 — an earlier version added a "complete the row set" scope from a foreman misreading of the owner's review.)* | C12 |
| C14 | `docs/in-numbers-cost-model` | Rework the operating half of `product/in-numbers.md` into a pricing/ROI basis (owner review 2026-08-19: the current form is "stats for nerds" — it never shows how the bill develops as demand grows, the reader has to piece it together). Every cost table ends with the same four standard rows: **fixed cost of the system · variable cost of the system · fixed cost per user · variable cost per user**. Drop invented total names ("ceiling total", "realistic total"). Add a cost-development view over user count (1 → 10 → 100 → 1,000) naming where each tier step lands, so unit economics and break-even are readable off the page. | C11 |
| C15 | `docs/in-numbers-readability` | Make the cost tables readable, not just correct (owner review 2026-08-20): resolve the €/$ split into one presentation currency with a stated dated rate (originals in footnotes); declare the user-count divisor in the summary heading; move the one-time ingestion row out of the monthly table; disambiguate "fixed" (system = doesn't scale with load; per user = marginal cost of one more user); move derivations and caveats out of the cost column into numbered footnotes. Structural fix: split each cost table into **line items** (item · volume · cost · footnote marker) plus a **summary block** headed with its user count. Applies page-wide, incl. the growth view. | C14 |
| C16 | `docs/readme` | Replace the day-0 README stub (411 bytes, no mention of what was built, no links to the live app or docs site) with the repo's front door — the artifact a cold reader meets first. Three jobs: what the product is and where to try it; what the seven days delivered vs what was deliberately cut; and how it was built — agent sessions under written briefs with the owner as proposer and quality gate, pointing at the dated owner attributions in this file and the review record. Verified quickstart. Links, never restates, facts owned by other documents. Owner-requested 2026-08-21. | C15; day-7 demo prep |
| C17 | `docs/parameter-rationale` | Add the **reasoning layer** to the retrieval/generation parameters (owner review 2026-08-21). The docs record what the constants *are* (RRF_K 50, pool 30, top-k 10, chunks 400/40, `vector(2000)`, history 12, …) but never why that shape is right — e.g. nothing says fusion exists because cosine and `ts_rank_cd` are on incomparable scales, or what K controls. Per parameter: what it trades off, what the alternative was, what would change it, and — the honesty rail — whether the value was measured, inherited from a documented pattern, or left at a default. Also surface the implicit choices: HNSW at pgvector defaults, and full-text hardcoded to the `'english'` configuration although the product ingests and speaks German. Extends the generative + logical views. | C9, C16 |
