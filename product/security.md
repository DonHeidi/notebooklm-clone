...
| SEC-4 | Platform (B1/B2) | **Spike route publicly reachable.** `/api/spike-stream` on the deployed container (B1's image) is unauthenticated on a public URL and proxies to the Generative-APIs account (token cost abuse). | Obscure URL; min-scale 0; max_scale 2 caps burn; Scaleway org rate limits. | B2's deploy of current main puts it behind A2's auth proxy automatically; A4 deletes the route. Verify with curl at B2 review (expect 307). |
