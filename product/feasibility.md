...
| Supabase free-tier: 500 MB DB / 500 MB RAM caps HNSW scale; project pauses after 1 week idle | Medium — **accepted (owner, B3 2026-08-18)** | Hosted project `ahphkkvsofqmxkqzbica` (eu-west-3) runs on **Free** by owner decision: $0, size caps fit prototype scale. The pause risk is accepted with an operational rule: **before any demo, check the dashboard and click Restore if paused (takes ~2 min), or upgrade to Pro ($25/mo) for the demo window.** A dead demo URL after ≥1 week idle is the expected failure mode, not a surprise |
